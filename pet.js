/**
 * ============================================================
 * canvas-pet engine - iframe-embeddable desktop pet
 * ============================================================
 * - Configurable via pet.config.js (window.DouknowPetConfig);
 *   falls back to DEFAULT_CONFIG below when a field is missing.
...
// 引擎默认配置（兜底）。运行配置来自 pet.config.js 的 window.DouknowPetConfig，
// 两者浅合并：缺字段用默认值，frames 对象单独再合并一次。删掉 pet.config.js 也能跑。
const DEFAULT_CONFIG = {
  ...
  quotes: [],
  // 宿主识别码：iframe 只接受 source === parentSource 的 postMessage
  ...
  frames: {
    ...
    // walk-right-1.png 必须直接就是「朝右」的帧，引擎不做运行时镜像。
...
const PET_CONFIG = Object.assign({}, DEFAULT_CONFIG, window.DouknowPetConfig || {});
PET_CONFIG.frames = Object.assign({}, DEFAULT_CONFIG.frames, (window.DouknowPetConfig || {}).frames || {});
...
class PetParty {
  ...
  bindParentBridge() {
    window.addEventListener('message', (event) => {
      ...
      if (!data || data.source !== PET_CONFIG.parentSource) return;
